...
[source]
----
include::./Examples/Eg020a_HelloWorld/src/Top.{SRC_EXT}[tag=all]
----
...
Now let's run the Bluesim simulation with the `-V` flag, or the IVerilog
simulation using the `+bscvcd` flag:
...
fact, it can start swaps as inputs are being streamed in.
...
Use Bluesim or Verilog sim, and generate a VCD waveform file (text
output the same as Version (a)).
...
Please feel free to peruse the Flute code at your leisure.  It is
written in BSV, not BH, but if you squint appropriately, it has the
same underlying syntactic structure as BH.  The two major parts of the
repo are:
...
used widely in hardware design and ARM used it in the AXI4 spec.
...
Please fix up that line to point at your clone of the _Flute_ repo.  Then:
...
will use `bsc` to compile all the code and link it into a Bluesim executable.
...
We have provided some pre-compiled RISC-V binaries; this diagram
illustrates how they were created, in case you wish to do it yourself
(later), with these, or modified, or other example programs.
